...
function fabDot(state) {
  ...
  var c = state === 'up' ? T.colorOk : state === 'down' ? T.colorCrit : T.colorMute
...
function fabPill(label, state, title) {
  var T = window.BPC.THEME
  var c = state === 'up' ? T.colorOk : state === 'down' ? T.colorCrit : T.colorMute
  var bg = state === 'up' ? 'rgba(63,185,80,0.10)' : state === 'down' ? 'rgba(248,81,73,0.12)' : 'rgba(255,255,255,0.05)'
  return '<span title="' + fabEsc(title || '') + '" style="display:inline-flex;align-items:center;gap:6px;'
    + 'padding:3px 9px;border-radius:12px;background:' + bg + ';border:1px solid ' + c + '33;font-size:.86rem;color:#CDD9E5">'
    + fabDot(state) + fabEsc(label) + '</span>'
}
...
function fabCompute(data) {
  ...
  CFG_FAB.spineOrder.forEach(function (spineName) {
    ...
    var cells = CFG_FAB.leafOrder.map(function (leafName) {
      ...
      return { leaf: leafName, state: !hit ? 'na' : (hit.up ? 'up' : 'down') }
    })
  ...
  var vpc = CFG_FAB.vpcPairs.map(function (pair) {
    var members = pair.members.map(function (leafName) {
      ...
      return {
        name:     leafName,
        present:  !!e,
        peerlink: !e || !pl ? 'na' : (pl.up ? 'up' : 'down'),
        keepalive:!e || !ka ? 'na' : (ka.up ? 'up' : 'down'),
      }
    })
  ...
  Object.keys(CFG_FAB.routerAttach).forEach(function (rname) {
    ...
    var fabricState = !upIfs.length ? 'na'
      : (upIfs.every(function (i) { return i.up }) ? 'up' : 'down')
