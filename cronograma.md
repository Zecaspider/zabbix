...
| Z.14 | P1 | **13 routers de agência com SNMP incompleto** — respondem a ICMP mas sem interfaces/CPU/memória monitorizadas (ponto cego no troubleshoot WAN) | ☐ | Auditoria 2026-06-27: 221 routers no g24, 208 com `net.if.status`, **13 sem**: `172.22.1.{132,160,203,213,240,38}` (ainda com hostname=IP), `RTADMHUMPATA00`, `RTAIAAN`, `RTFAASU00.bpc.intranet`, `RTKIKOLO02`, `RTPOSTO_IRI_00`, `RTSICA00`, `RT_SME_VIANA`. Todos no grupo "Discovered hosts" → aplicar templates SNMP + corrigir credenciais/walk + renomear os 5 com IP. O N4 mostra "Sem SNMP" nos painéis afectados (dashboard correcto; é dado em falta no Zabbix). **Nota:** mesmo agency routers "completos" (ex. CUNHINGA) podem ter `net.if.status` com `lastclock=0` (nunca recolhido) → sem histórico pré-queda. Verificar SNMP walk das interfaces nos agency routers em geral |
| Z.15 | P2 | **Monitorização per-spoke no hub DMVPN** (`DC1-RTE-WAN-AG`) — NHRP/crypto session por agência | ☐ | O hub só tem estado **por provider** (Tu101 UNITEL, Tu105 MST…), não por agência. Adicionar items NHRP/IPsec por spoke daria visibilidade do túnel de cada agência **mesmo com o router da agência down** (causa raiz de agências sem sistema). Habilita 9.6 completo |
