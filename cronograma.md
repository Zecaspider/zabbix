...
Última actualização: 2026-06-29 (Gap A GPS geomap: auditoria completa dos 221 hosts g24 → 49 sem coordenadas → 46 populados via API Zabbix (45 confirmados + RT_UBM30 Km30) → geomap passa 172→218. 3 IPs anónimos (`.38`/`.203`/`.240`) identificados como hosts manuais DOWN há semanas sem tags/inventory/Excel → aguardam identificação pela equipa de rede. Artefactos: `BPC/audit_gps_g24.csv`, `BPC/hosts_sem_gps_preenchido.csv`. ANTES: 9.6 v1 painel Operadoras WAN hub DMVPN)
