...
> Última revisão: 2026-07-13 (arquivamento das Versões A/B do N3 — a ficha híbrida
> `vm-n3-ficha` passa a único N3 canónico — ver §13.8)
...
- [x] Criar painel L3 de top processos por CPU/RAM — **PILOTO 2026-07-13** (host nomeado e aprovado pelo utilizador): item `proc.get[,,,summary]` criado na `VS8000345` (itemid 542814, texto, 2m, history 3d, SEM triggers → zero risco de flood) + painel `n3-hibrido/l3h-processos.js` na ficha híbrida. CPU% por processo = Δ(cputime_user+system) entre as 2 últimas amostras do histórico (`history.get` com `history:4` — o item é texto) / Δt / nCores; RAM = wkset (KB). Campos Windows confirmados ao vivo: name, processes, threads, handles, wkset, vmsize, cputime_user/system, io_*, page_faults. Validado: top CPU (svchost 3.7%, WmiPrvSE 3.2%) e top RAM (svchost 1.1 GB = 13.9% de 8 GB). Limitação conhecida: `VS8000345` não tem item de nº de cores → rodapé diz honestamente "% de 1 core". **Alargar a outras VMs = criar o mesmo item lá (aprovação por host, ou template próprio `BPC Top Processos` se generalizado) — nunca mexer no template Windows partilhado**. **PRODUÇÃO 2026-07-13 (aprovado)**: template **`BPC Top Processos`** (14721, grupo Templates/Operating systems) criado — master `proc.get[,,,summary]` 5m/**history=0** (não persiste) + dependente `bpc.proc.top12` (preprocessing JS, união top-12 wkset + top-12 cputime, ~2.3 KB medidos, **85% menos** que o raw) history 7d, **sem triggers**. Aplicado via `host.massadd` a **148 VMs** = grupo 609 ∩ `ambiente≈produ*` ∩ agente vivo (`agent.ping` < 2h); as 129 de produção sem agente ficaram de fora de propósito (items nasceriam mortos). Custo total estimado: ~0.5 GB/7d na frota. A VS8000345 (QA) mantém os items directos do piloto. O painel `l3h-processos.js` prefere `bpc.proc.top12` com fallback ao raw — funciona em ambos. **FIX 2026-07-13 (2ª ronda, achado do utilizador na VS8000218)**: `proc.get` só existe no agente Windows ≥6.4 — 60 dos 148 hosts (agentes 6.0/6.2) ficaram `Unsupported item key`. Master do template trocado para **`wmi.getall[root\cimv2, select ... from Win32_Process]`** (universal, qualquer agente Windows) com preprocessing v2 que agrega por nome (WorkingSetSize bytes→KB, cputime 100ns→s) e produz o MESMO schema — painel intocado. Nota: com `history=0` o `lastclock` do master nunca actualiza — monitorizar a frota pelo DEPENDENTE, não pelo master. Primeiro fruto do coletor: `httpd.exe` na VS8000218 com **15.006 threads** e 6.9M s de CPU acumulado (leak de threads no Apache — a causa dos 100% CPU/fila 69 daquela VM), escalar ao dono do sistema SOPHOS/DTI
...
### 13.0 N3 híbrida (`vm-n3-ficha`) — **única versão canónica desde 2026-07-13** (§13.8)
...
Nova pasta `n3-hibrido/` (manifest próprio; coexistiu com as Versões A e B
entre 2026-07-13 de manhã e a tarde do mesmo dia, quando foram arquivadas —
ver §13.8):
...
### 13.1 [HISTÓRICO — arquivado 2026-07-13, ver §13.8] Duas versões N3 em produção

> Secção mantida como registo histórico do período em que A/B estiveram em
> produção lado a lado com o híbrido (manhã de 2026-07-13). **Já não reflecte
> o estado actual** — as duas versões abaixo foram arquivadas nesse mesmo dia
> (§13.8); o único N3 canónico é o híbrido (§13.0).

O **N3 · VM Detalhe** existiu em **duas versões, ambas em produção**. Partilhavam os
mesmos `l3-*.js` (o render é idêntico); diferiam no tipo de painel Grafana:

| Versão | UID | Manifest / snapshot | Natureza |
|---|---|---|---|
| **A — all-BT** | `0ae673a3-44c8-41e0-98f5-f5c53473ad54` | `arquivo-n3-versoes-a-b/manifest.json` + `dashboard-completo.json` | 9 painéis Business Text |
| **B — nativo** | `0812353b-3da2-4b65-a884-862633c7d70a` | `arquivo-n3-versoes-a-b/manifest-versao-b.json` + `versao-b-corrected.json` | gauges/stat/timeseries nativos Zabbix + 4 painéis BT (build: `build_versao_b.py`) |

Ambas viveram na pasta Grafana `03·Servidores Virtuais` (`cfpm0sdsxjb40c`) até 2026-07-13, quando foram movidas para `99·Arquivo` (`dfpm0sey9ut4wb`) — ver §13.8.
...
- ~~`n3/build_versao_b.py` tem path de saída stale~~ — irrelevante desde 2026-07-13, Versão B arquivada (§13.8).
- ~~Migrar mais painéis all-BT para nativo (Versão B)~~ — decisão de arquitectura fechada em 2026-07-13: o híbrido é o único caminho a seguir (§13.8), não há mais migração A→B.
...
### 13.8 Arquivamento das Versões A e B — híbrido passa a único N3 canónico (2026-07-13)

Pedido directo do utilizador, depois de o híbrido (`vm-n3-ficha`, §13.0) ter
estabilizado (Top Processos em produção nas 148 VMs, ver `handoff-vm-ficha-hibrida-20260713.md`):
"arquiva as outras versões n3, e manter o actual como canónico".

**Executado:**
1. **Grafana**: os 2 dashboards movidos de `03·Servidores Virtuais`
   (`cfpm0sdsxjb40c`) para `99·Arquivo` (`dfpm0sey9ut4wb`) via
   `POST /api/dashboards/db` (`folderUid` explícito + `overwrite:true`,
   mesma técnica do achado 2026-07-10 registado no `CLAUDE.md` raiz —
   `push_panel.py`/`push_native.py` não teriam forçado a pasta). Nenhum dos
   dois dashboards foi apagado — continuam acessíveis pelo UID, só mudaram
   de pasta. Versão A: `0ae673a3-…`, version 45→46. Versão B: `0812353b-…`,
   version 14→15.
2. **Local**: pasta `n3/` renomeada para `arquivo-n3-versoes-a-b/` (mesmo
   nível da pasta `n2/`/`n3-hibrido/`, seguindo o padrão já usado em
   `04-rede/04.2-borda-dc/arquivo-borda-dc/`). Conteúdo interno preservado
   tal e qual, incluindo a subpasta `arquivo-n3/` (scaffolding de 2026-07-09,
   já arquivado antes disto). `manifest.json` e `manifest-versao-b.json`
   dentro da pasta arquivada tiveram o `folderUid` actualizado para
   `dfpm0sey9ut4wb`, para que um push acidental não os devolva a
   `03·Servidores Virtuais`. Novo `README.md` em
   `arquivo-n3-versoes-a-b/README.md` documenta o arquivamento.
3. **Este `CLAUDE.md`**: §13.0 e §13.1 actualizadas para reflectir que o
   híbrido é agora o único N3 canónico (§13.1 mantida como registo
   histórico do período de coexistência); item de §13.3 sobre migração
   A→B fechado (deixou de fazer sentido).

**Não corrigido nesta ronda (âmbito do pedido era só arquivar, não caçar
consumidores)**: 2 ficheiros do domínio `07-apis` ainda apontam para a
Versão A (`07-apis/n3/n3-app-vms-tabela.json`, drill "Detalhe da VM"; e
`07-apis/n4-sistema/l4-sys-vms.js`, `CFG_S4VMS.svN3Url`). O link continua a
funcionar (o dashboard só mudou de pasta), mas passa a levar a um
dashboard arquivado em vez do híbrido. Corrigir para `vm-n3-ficha` fica
pendente — precisa da mesma troca de variável (`?var-hostid=<técnico>` em
vez de `?var-hostid=<visível>`) já feita no `l2-tabela.js` deste domínio
(§13.0), e de confirmar se o campo `"VM"` que a tabela de `07-apis` usa já
é o nome técnico ou o visível antes de alterar.
