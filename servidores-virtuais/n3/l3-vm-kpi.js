(function () {
  ...
  // ╔══════════════════════════════════════════════════════════════════════════╗
  // ║  D5-Detalhe-VM · ROW 1 · KPI GOLDEN SIGNALS  v16.0                     ║
  // ║  Autor: BPC                                                              ║
  // ║                                                                          ║
  // ║  ALTERAÇÕES v15 → v16:                                                  ║
  // ║                                                                          ║
    ...
    cpu: function (items, byKey, hasAgentIface) {
      ...
      if(r.source==='agent'){
        ...
        return{value:r.value,source:'Agente',hasAgent:hasAgent,
          ...
          sub2:'fila '+(qI?parseFloat(qI.lastvalue).toFixed(0):'—'),sparkItem:r.item};
      }
      if(r.source==='vmware'){
        ...
        return{value:r.value,source:'VMware',hasAgent:false,
          ...
          sub2:'latência '+(lI?parseFloat(lI.lastvalue).toFixed(2)+'%':'—'),sparkItem:r.item};
      }
  ...
  var CSS = (function () {
    ...
    return [
      ...
      '#bt-kpi-gs .ks-grid{display:grid;grid-template-columns:1fr 1fr 1fr 1fr 2fr;gap:8px;height:100%;}',
  ...
  var Card = {
    ...
    health: function (d) {
      ...
      return '<div class="ks-card hlth">'
        ...
        +(vmwHtml?'<div class="ks-sep"></div>'+vmwHtml:'')+warnHtml+'</div>';
    },
    ...
    cpu: function (d) {
      ...
      return '<div class="ks-card cpu">'
        ...
        +SVG.infoBox([['fonte',d.source,srcColor],['contexto',d.sub1||'—',C.sub]])
        +'</div>';
    },
    ...
    disk: function (d) {
      ...
      function dkKv(val,thrKey,label,fmtFn,unit){
        var t=CFG.thresholds[thrKey]||{warn:50,crit:100};
        var cls=U.thrClass(val,t),color=U.thrColor(val,t);
        var vStr=U.isValid(val)?fmtFn(val):'—';
        return '<div class="ks-dk-kv">'
          +'<span class="ks-dk-val" style="color:'+color+';">'+U.esc(vStr)+'</span>'
          +'<span class="ks-dk-badge '+cls+'">'+U.esc(label)+'</span>'
          +'<span class="ks-dk-unit">'+U.esc(unit)+'</span>'
          +'</div>';
      }

      var ioHtml = (d.source==='VMware'||d.source==='—')
        ? '<div style="color:'+C.sub+';font-size:9px;grid-column:1/-1;text-align:center;padding:4px 0;">I/O indisponível · sem agente</div>'
        : dkKv(d.ioW,    'ioOps','ESCRITA', function(v){return v.toFixed(1);},'w/s')
        + dkKv(d.ioR,    'ioOps','LEITURA', function(v){return v.toFixed(1);},'r/s')
        + dkKv(d.ioWt,   'ioLat','LAT.ESC', function(v){return v.toFixed(2);},'ms')
        + dkKv(d.ioRt,   'ioLat','LAT.LT.', function(v){return v.toFixed(2);},'ms')
        + dkKv(d.ioQueue,'ioQ',  'QUEUE',   function(v){return v.toFixed(2);},'depth')
        + dkKv(d.ioUtil, 'ioU',  'UTIL.',   function(v){return v.toFixed(1)+'%';},'% idle');
      ...
      return '<div class="ks-card io">'
        ...
        +'<div class="ks-dk-sep"></div>'
        +'<div class="ks-dk-io">'+ioHtml+'</div>'
        +'</div>';
    },
  };
  ...
  console.log('[KPI-GS v16] host:', hostName, '| token anterior:', _ns.token || 'nenhum');
  ...
  if (_prevController) {
    _ns.abortTimer = setTimeout(function () {
      ...
      console.log('[KPI-GS v16] fetch anterior abortado (delayed)');
      _ns.abortTimer = null;
    }, CFG.abortDelayMs);
  }
  ...
  if (!hostName) {
    console.log('[KPI-GS v16] limpando DOM para', hostName, '| sou current:', _isCurrent());
  ...
  ZbxApi.getHostId(hostName, signal)
    .then(function (hostInfo) {
      if (!_isCurrent()) {
        console.log('[KPI-GS v16] descartado após getHostId →', hostName);
        return;
      }
      ...
      return ZbxApi.getItems(hostInfo.hostid, signal)
        .then(function (items) {
          if (!_isCurrent()) {
            console.log('[KPI-GS v16] descartado após getItems →', hostName);
            return;
          }
          ...
          return Promise.all([
            ...
            console.log('[KPI-GS v16] render OK →', hostName,
    ...
    .catch(function (e) {
      if (e.name === 'AbortError') {
        console.log('[KPI-GS v16] AbortError (esperado em mudanças rápidas) →', hostName);
        return;
      }
      if (!_isCurrent()) return;
      console.error('[KPI-GS v16] Erro:', e.message);
