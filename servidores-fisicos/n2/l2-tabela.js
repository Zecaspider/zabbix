// ╔══════════════════════════════════════════════════════════════════════════╗
// ║  BPC NOC — N2 · SERVIDORES FÍSICOS (ESXi) · TABELA DE HYPERVISORES     ║
// ║  v3.0 · Framework BPC-UI v9 · waitForBPC bootstrap                     ║
// ║                                                                          ║
// ║  Colunas: Estado · Host (tech+vis) · Cluster · CPU(%+spark) ·          ║
// ║           RAM(%+GB) · VMs · Uptime · Power · Datastores · Triggers · → ║
// ║  Fontes NOC @ 3m · Ordenação: pior estado primeiro                      ║
// ║                                                                          ║
...
var CFG_TAB = {
  ...
  thresholds: {
    cpu: { warn: 75, crit: 90 },
    ram: { warn: 70, crit: 85 },
    ds:  { warn: 20, crit: 10 },   // % livre (invertido: baixo = mau)
  },

  // Fontes NOC legíveis a 3m
  fs: {
    header:   '13px',
    cell:     '15px',
    value:    '18px',
...
function tabGetHosts(rpc) {
  if (_tabHostCache && _tabHostCache.length) return Promise.resolve(_tabHostCache)
  return rpc('host.get', {
    groupids:  [CFG_TAB.groupId],
    selectTags: 'extend',
    output:    ['hostid', 'name', 'host'],
    filter:    { status: 0 },
  }).then(function (all) {
    var tag = CFG_TAB.hostTag
    _tabHostCache = all.filter(function (h) {
      return /^VIRT\s*-\s*ESXi\s*-/i.test(h.name) &&
        (h.tags || []).some(function (t) { return t.tag === tag.tag && t.value === tag.value })
    })
    return _tabHostCache
  })
}

function tabGetSparklines(rpc, cpuByHost) {
  var ids = Object.keys(cpuByHost).map(function (hid) { return cpuByHost[hid] }).filter(Boolean)
  if (!ids.length) return Promise.resolve({})
  var tf = Math.floor(Date.now() / 1000) - CFG_TAB.sparkSeconds
...
function tabFetch(rpc) {
  return tabGetHosts(rpc).then(function (hosts) {
    if (!hosts.length) return { hosts: [], rows: [] }
...
function tabCompute(data, sparks) {
  var S = window.BPC.state
  var hosts   = data.hosts
  var items   = data.items
  var triggers = data.triggers

  // Agrupar triggers por host
  var trgByHost = {}
  ...
  var rows = hosts.map(function (host) {
    ...
    var stale   = cpuAge > CFG_TAB.maxAgeSec
    var noData  = cpu == null

    var cpuState  = (cpu != null && !stale) ? S.metric(cpu, CFG_TAB.thresholds.cpu) : (stale ? 'warn' : 'ok')
    var ramState  = ramPct != null ? S.metric(ramPct, CFG_TAB.thresholds.ram) : 'ok'
    ...
    var dsState = dsWorst ? (dsWorst.freePct <= CFG_TAB.thresholds.ds.crit ? 'crit' : dsWorst.freePct <= CFG_TAB.thresholds.ds.warn ? 'warn' : 'ok') : 'ok'
    ...
    // Technical name = h.host (ex: sv9000640)
    // Visible name = remove prefix (ex: VIRT - ESXi - sv9000640 → sv9000640)
...
function tabRender(el, rows, err) {
  ...
  var TD_STYLE = 'padding:8px 10px;vertical-align:middle;border-bottom:1px solid rgba(255,255,255,0.06)'
  ...
  var thead = '<tr>'
    ...
    + '<th style="' + TH_STYLE + '">VMs</th>'
    + '<th style="' + TH_STYLE + '">Uptime</th>'
    + '<th style="' + TH_STYLE + '">Power</th>'
    + '<th style="' + TH_STYLE + '">Datastores</th>'
    + '<th style="' + TH_STYLE + '">Triggers</th>'
  ...
  var tbody = rows.map(function (r) {
    ...
    // Estado
    var colEstado = '<td style="' + TD_STYLE + '">' + stateDot(r.rowState) + '</td>'

    // Host: tech name + visible name
    var colHost = '<td style="' + TD_STYLE + ';min-width:140px">'
    ...
    // Cluster
    var colCluster = '<td style="' + TD_STYLE + ';font-size:' + FS.cell + ';color:var(--bpc-mute)">' + SH.esc(r.cluster) + '</td>'

    // CPU: % + barra + sparkline
    var cpuColor = S.color(r.cpuState)
    var colCpu = '<td style="' + TD_STYLE + ';min-width:90px">'
    ...
    var colRam = '<td style="' + TD_STYLE + ';min-width:110px">'
    if (r.ramPct == null) {
    ...
    // VMs
    var colVms = '<td style="' + TD_STYLE + ';text-align:center">'
    ...
    // Uptime
    var colUptime = '<td style="' + TD_STYLE + ';font-size:' + FS.cell + ';color:var(--bpc-mute);white-space:nowrap">'
      + fmtUptime(r.uptime) + '</td>'

    // Power
    var colPower = '<td style="' + TD_STYLE + ';font-size:' + FS.cell + ';color:var(--bpc-mute);white-space:nowrap">'
      + (r.power != null ? r.power + ' W' : '—') + '</td>'

    // Datastores: pior free% + lista top 2
    var colDs = '<td style="' + TD_STYLE + ';min-width:110px">'
    if (!r.dsArr || !r.dsArr.length) {
      colDs += '<span style="color:var(--bpc-mute);font-size:' + FS.cell + '">—</span>'
    } else {
      var dsColor = S.color(r.dsState)
      colDs += '<span style="font-size:' + FS.value + ';font-weight:700;color:' + dsColor + ';font-family:monospace">'
        + r.dsArr[0].freePct.toFixed(1) + '%</span>'
    ...
    // Triggers
    var colTrg = '<td style="' + TD_STYLE + ';min-width:80px">'
...
function startTabela(rpc) {
  window.BPC.utils.waitForElement(CFG_TAB.elementId, function (el) {
    ...
    function load() {
      tabFetch(rpc).then(function (data) {
        var cpuByHost = {}
        ;(data.items || []).forEach(function (i) {
          if (i.name === 'CPU usage in percent') cpuByHost[i.hostid] = i.itemid
        })
        tabGetSparklines(rpc, cpuByHost).then(function (sparks) {
          tabRender(el, tabCompute(data, sparks), null)
        })
      }).catch(function (e) {
        tabRender(el, null, e.message || String(e))
      })
    }
...
function initWithRetry(attempt) {
  ...
  if (attempt > 50) { console.error('[BPC] l2-tabela servidores-fisicos: waitForBPC indisponivel'); return }
