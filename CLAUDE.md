...
| **Reorganização de pastas pendente** | 12 dashboards de produção estão na pasta `General` em vez das pastas de domínio correctas. Nenhum domínio conta como "pronto" enquanto os seus dashboards estiverem em `General`. DoD e checklist de conclusão em `documentacao/engenharia-do-sistema.md §4.2`. Esta reorganização é uma sessão dedicada — não executar a meio de outra tarefa. |
