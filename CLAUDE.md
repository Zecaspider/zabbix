...
> - **Fora desta pasta** — `C:\Repositorios\zabbix\bpc-workspace\handoff-bases-dados-classificacao-20260712.md` — **PONTO DE ENTRADA** para continuar a Fase 6 (`cronograma.md` 6.1–6.1.4): N2 Bases de Dados migrado (`bd-n2`, MSSQL, KPI + tabela por host), 47 hosts com `camada=base de dados` auditados (39 em Produção, só 1 com dados reais), 4 hosts mal classificados corrigidos ao vivo (3 NiFi → `camada=integração`, correcto) — **mas `VS8000928` pode ter sido corrigido para o lado errado** (nota de inventário sugere Postgres/MySQL real, não "integração" — confirmar motor antes da próxima sessão). Tarefa combinada pendente: rever as ~35 VMs de Produção restantes uma a uma, confirmar motor de BD real (MSSQL/Postgres/MySQL/nenhum) e aplicar tag+template certos, antes de decidir a migração de `BPC MSSQL by ODBC` (server-side, quebra em portas não-standard) para `MSSQL by Zabbix agent 2` (liga a `localhost`, 0 hosts a usar ainda, templates PostgreSQL/MySQL by Zabbix agent 2 também disponíveis)
