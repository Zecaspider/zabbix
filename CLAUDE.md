...
| **Fluxo Borda DC ("Borda DC", segmento N2)** | O segmento chamado **"WAN"** no N2 é especificamente os **5 routers de borda do DC** (`HG_DC_ROUTERS`, g27: `DC1-RTE-WAN-INT/WAN-EMIS/WAN-AG/PARC/GTW01`) — renomeado para **"Borda DC"** (2026-07-01) para não confundir com a WAN das próprias Agências (220 routers, também WAN, mas segmento à parte — `04.1·Agências`). **2 fluxos paralelos** (revisão 2026-07-01, confirma inversão da decisão D2 de `mockups-rede.md`, que planeava apagar a vista por provedor — ficou mantida por ter valor operacional próprio de accountability de SLA): `N2 Rede → N3 WAN Serviços (rede-n3-wan, pasta 04.2·Borda DC, 01-n3-rede-wan/, 5 cards por router) → N4 WAN Router (rede-n4-wan-router, 02-n4-rede-wan-router/, ficha técnica canónica: circuitos por categoria, IP SLA, alertas nativos)` — triagem por router/serviço; e em paralelo `N3 WAN Provedores (rede-n3-wan-carriers, 03-n3-rede-wan-carriers/) → N4 WAN Provedor (rede-n4-wan-provedor, 04-n4-rede-wan-provedor/, tabela de circuitos por operadora)` — accountability de SLA por carrier (Unitel/MST/AT/ITA/Multitel/IPWORLD/Connectis/Azure). Numeração de pasta reinicia por fluxo (par 01→02, par 03→04) — nunca misturar pastas de fluxos diferentes fora desta ordem. **Nenhum dos dois fluxos tem N5**: ambos os N4 já são fichas de dispositivo/provedor único (5 routers no total) sem camada adicional por baixo a justificar — diferente de Agências/Edifícios, onde o N4 agrega vários dispositivos por site. Não há router "principal" entre os 5 — são paralelos, cada um com papel distinto (Internet/EMIS/hub-agências/parceiros/gov). **Regra de isolamento por segmento (2026-07-01):** cada dashboard vive só na pasta do seu segmento — nunca partilhado entre pastas. O `rede-n4-wan-dispositivo` (genérico, aceita qualquer host) foi **movido de Borda DC para `04.1·Agências`** por só ter consumidor real ali (`l4-ag-ficha.js`, botão "Interfaces WAN" da ficha de agência); Borda DC não usa nenhum dashboard genérico. Se um caso de uso futuro precisar de uma vista "dispositivo genérico" dentro de Borda DC, duplicar o dashboard com um UID novo — nunca reaproveitar o de Agências. |
