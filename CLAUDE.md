...
| **Naming de dashboards** | Título: `Nx · Domínio [· Âmbito] — Propósito` (prefixo Nx obrigatório para ordenação por nível; sentence case; ` · ` entre hierarquia; ` — ` antes do propósito). UID: `dominio.nivel.funcao` — **sem número de ordenação** (número só nas pastas/ficheiros para ordenação visual). Mapa canónico Rede em `documentacao/engenharia-do-sistema.md §4.0`. Migração UIDs pendente (T-04). |
| **Naming de directorias locais** | Pastas de domínio: `NN-nome/` espelhando numeração Grafana (ex: `04-rede/`). Subpastas: `NN-nivel-dominio[-subdomain]/` onde NN é a ordem de drill (ex: `02-n3-rede-agencias/`). Mapa completo em `documentacao/engenharia-do-sistema.md §4.1`. Migração de pastas pendente (T-05 — mesma sessão que T-04). |
