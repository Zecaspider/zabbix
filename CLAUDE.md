...
> - **`documentacao/taxonomia-grupos-tags.md` — CONTRATO CANÓNICO de classificação de hosts (aprovado 2026-07-15)**: regra de ouro (grupo = domínio com pasta Grafana, `BPC/DOMINIO/NN Nome` em ASCII; tudo o resto = tags; subdomínio = tag `tipo`), racional domínio-vs-camadas, modelo-alvo de **9 grupos DOMINIO** (inclui `08 Datacenter Fisico` novo para IBM Power/UCS/PowerFlex/PDUs, `10 Servicos de Suporte`, e **`09 Integracao e APIs`** novo — EAI+Integrador ESB, decisão §8 de 2026-07-18), invariantes verificáveis (host activo ⇒ ≥1 grupo DOMINIO; VM ⇒ sempre em 03 + funcional), catálogo de tags canónicas, plano Fase A (colapso AMBIENTE/CAMADA/TECNOLOGIA/LOCALIZACAO, 0 refs em dashboards) / Fase B (renames+fusões+split do 603, refs medidas por grep: 609→32 ficheiros, 602→17) e decisões de classificação para casos futuros (OKD fica em 03, VMs de segurança em 03 por tag, Power em 08). **§8 (2026-07-18)**: fronteira Negócio/Integração/Suporte fechada — `07` renomeado "APIs e Servicos de Negocio"→"**Servicos de Negocio**" (método de monitoria vira tag `fonte`), `09 Integracao e APIs` novo, OCP/OKD ficam em 03+tag, GitLab e ELK-de-logs→10; executado via `aplicar_dominio09_20260718.py --apply`. **Ler antes de criar/classificar qualquer host**
> - **`documentacao/reconciliacao-dominios-taxonomia-20260714.md` — registo histórico da reconciliação Grafana↔Zabbix (2026-07-14) que originou o contrato acima**: registo da reconciliação Grafana↔Zabbix de 2026-07-14 — decisões F1 aprovadas (grupos = só eixo de domínio; tags ganham aos grupos CAMADA; grupos SERVICO/* vão colapsar para a tag `servico`), limpeza F2 (tags `ambiente`/`camada` normalizadas, 5 tags `NN camada` apagadas, grupos 490/632 apagados) e reclassificação F3 (91 hosts órfãos → 2 por decisão consciente; `Novos_Inventario` extinto). Pendentes: F3.5 (colapso SERVICO), 15 hosts A-CLASSIFICAR, host `BNA`, router novo `10.10.205.55` no HG_DC_ROUTERS (Network, criado 2026-07-14 — quebra a premissa "5 routers" da Borda DC). Ler antes de criar/classificar hosts novos ou mexer em grupos/tags
...
> - `documentacao/metodologia-auditoria-sistema.md` — checklist transversal para "auditoria profunda do sistema"/"relatório de estado" (Zabbix Infra+Network+Grafana, 7 etapas, só leitura + plano de limpeza aprovado caso a caso); inclui as armadilhas de API que já falsificaram auditorias (`webitems:true`, nomes de problema congelados, desactivar≠fechar) e o prompt exacto a usar. Origem: auditoria+limpeza de 2026-07-13 (Z.53)
...
> - **`documentacao/incidente-plugin-zabbix-grafana-20260710.md` — RUNBOOK a usar sempre que os dashboards Zabbix ficarem sem dados depois de um corte de energia/reboot do servidor Grafana**: diagnóstico em 3 passos sem SSH (health do Grafana → API do Zabbix → health dos datasources), causa raiz confirmada (plugin `alexanderzobnin-zabbix-app` desaparece de `/api/plugins` por escrita interrompida no disco) e o fix (`grafana-cli plugins remove/install` + `systemctl restart grafana-server`, já em `_toolkit/metodologia/troubleshoot.md`)
...
> - **`documentacao/reconciliacao-50-sistemas-excel.md` — PONTO DE ENTRADA para a Fase 7.0.5**: tabela mestra dos 50 sistemas do relatório diário × `app-*` × VM, cobrindo de uma vez os objectivos (1) app-*+VM mapeados/tagueados, (2) tags iguais em sintético e VM, (3) multi-VM identificado e mapeado, (4) monitoria activa nos dois lados — ler antes de criar `app-*` novos, decidir a tensão N:M da `VS8000305` (§3), ou fechar qualquer gap da lista
...
> - **Fora desta pasta** — `C:\Repositorios\zabbix\bpc-workspace\handoff-vm-ficha-hibrida-20260713.md` — **PONTO DE ENTRADA para continuar Servidores Virtuais** (cronograma 3.11-3.12): ficha híbrida `vm-n3-ficha` promovida a N3 canónico (hero + sinais vitais com pulso + serviços servidos pela VM + top processos + USE/RED nativos + ficha 3 cartões), template `BPC Top Processos` em produção em 148 VMs (padrão master efémero history=0 + dependente trimmed via preprocessing JS, 0.61 GB/7d medidos), header-lite canónico novo em `_comum/header-lite.js`, e as lições de API que generalizam (filtro de host das queries de métricas do datasource Zabbix = nome TÉCNICO em regex, via variável MySQL `__text`/`__value`; `webitems:true` obrigatório para items de web scenario; CPU por processo = Δcputime entre 2 amostras do histórico)
...
> - **`documentacao/plano-melhorias-observabilidade-20260712.md` — PONTO DE ENTRADA para a Fase 17 (melhorias transversais, 2026-07-12)**: relatórios Excel/PDF por domínio (`relatorios/por-dominio/gerar_relatorio.py`, PDF via Edge headless porque o `grafana-image-renderer` não está instalado), dashboard `visao-notificacoes` (cadeia action→mediatype→destinatários→entregas lida do **MySQL via ds/query** — `action.get` é bloqueado no proxy zabbix-api; achado central: **21.878 alertas de email em 7 dias, 100% falhados** — action 9 ON + mediatype Email OFF desde o incidente GLPI), índices de dashboards (`visao-indice-dashboards` BT + `visao-indice-nativo` dashlist), domínio novo `10 · Serviços de Suporte` (`suporte-n2`, por tag `servico`, zero escrita Zabbix) e N1 v3.0 — pendentes em `cronograma.md` 17.6
> - `06-bases-dados/CLAUDE.md` — **regras e desenho deste domínio**: modelo de "fonte de dados" (tier ODBC/Perfmon/Serviço/Sem sinal — o mecanismo central que faz os dashboards mostrarem sinal real mesmo sem credencial SQL, e "sobem" sozinhos para o conjunto completo assim que a credencial existir), achados operacionais reutilizáveis (17 hosts MSSQL confirmados, 3 em instância nomeada, 1 Oracle parado, `PRISIGEP` a 99% de log), lições técnicas de Zabbix (chave certa `service.info` não `service_state`, quirk do nome WMI sem underscore, `lld_macro_paths` obrigatório em discovery `wmi.getall`), e o mockup ASCII aprovado do N2 redesenhado + N3 novo (código ainda por escrever, ver `cronograma.md` 6.2)
> - **Fora desta pasta** — `C:\Repositorios\zabbix\bpc-workspace\handoff-bases-dados-classificacao-20260712.md` — **PONTO DE ENTRADA** para continuar a Fase 6 (`cronograma.md` 6.1–6.1.7): N2 Bases de Dados migrado (`bd-n2`, MSSQL, KPI + tabela por host). Tarefa combinada de rever as VMs de Produção uma a uma **concluída** (2ª sessão, 2026-07-12) — `camada=base de dados`+`ambiente=Produção` passou de 39 para 23 hosts (16 corrigidos ao todo: 3 NiFi + `VS8000928` numa 1ª sessão, mais 12 hosts numa 2ª sessão via evidência real — porta TCP + erro ODBC + `Get-Service`/`Get-NetTCPConnection` por WinRM em hosts nomeados). Dos 23 restantes: 21 com motor confirmado (MSSQL ou Oracle — 3 em instância nomeada, 1 Oracle com o serviço **parado**, achado operacional) e 2 por confirmar (`SV9000401`/`VS9000404`, WinRM inacessível). **Pendente**: `VS8000928` (alerta antigo, nunca resolvido), tag `motor` (mssql/oracle/nenhum, ainda não existe), credencial `zbx_monitor` em 16 hosts, migração de template para os 3 de instância nomeada, e confirmar se `VS8000821`/`VS8000823`/`VS8000850`/`VS8000851` (sem nenhum software aplicacional instalado) estão mesmo em uso — ver secção "Actualização" no fim do handoff para o detalhe completo

> - **`documentacao/reconciliacao-cobertura-e-inventario-20260717.md` — PONTO DE ENTRADA para a Fase 18 (reconciliação inventário + estratégia de cobertura)**: desfaz a colisão de nomenclatura entre os 3 eixos (dashboards N1-N5 · fonte de dados infra/SO/funcional · profundidade do check funcional web L1-L4); mapeia a "estratégia L1-L4" rascunhada (SUPERSEDED, `inventario/estrategia-cobertura.md`) no vocabulário do projecto; regista o trabalho de saneamento de tags/inventário de 2026-07-16/17 (`inventario/README.md`+`pendentes.md`+`relatorio-agentes-producao.md`, cronograma Fase 18); e o **bug identificado do `BPC DNS Check`** (`net.dns` posto como Simple check, é item de Zabbix agent — fecha o §4 do `incidente-dns-externo-20260716.md`). Ler antes de retomar cobertura de serviços/DNS/BD ou de mexer no inventário de VMs
...
|---|---|
| **Pastas Grafana por domínio** | Cada dashboard vive na pasta de domínio do seu nível, não numa pasta única (reorg Fase 12, 2026-06-19). UIDs das pastas: `00·Visão Geral` `bfpm0sdaos074d` · `01·Infraestrutura VMware` `bfpm0sdhhi22od` · `02·Armazenamento` `dfpm0sdnq8x6oe` · `03·Servidores Virtuais` `cfpm0sdsxjb40c` · `04·Rede` `bfpm0sdxiclxcf` · `05·Segurança` `afpm0se1lombkb` · `06·Bases de Dados` `afpm0se5rij28d` · `07·APIs e Serviços de Negócio` `bfpm0sedbpgqob` · `08·Datacenter Físico` `efsh9mn66m4u8b` (**criada 2026-07-18** — IBM Power/UCS/PowerFlex/HMC; N2 `datacenter-n2`; local `08-datacenter-fisico/`) · `09·Integração e APIs` `dfsh9jx5gjzswd` (**criada 2026-07-18**, §8 taxonomia — EAI/Integrador ESB; N2 `integracao-n2`; local `09-integracao-apis/`) · `10·Serviços de Suporte` `dominio-suporte` (**criada 2026-07-12**, Fase 17 — DNS/DHCP/AD/NTP/WSUS/Email/IAM; local `10-servicos-suporte/`) · `99·Arquivo` `dfpm0sey9ut4wb`. **`08·Serviços de Negócio` apagada (2026-07-10)** — estava vazia (nunca chegou a ser construída; eBankit sempre viveu dentro de `07` como mais uma app sintética) e redundante com `07`; a pasta `07` foi renomeada de "APIs e Serviços" para "**APIs e Serviços de Negócio**" para reflectir a fusão dos dois conceitos, sem obrigar a renomear os títulos dos dashboards existentes (continuam "N2 · APIs e Serviços" etc.). Local: pasta `08-servicos-negocio/` removida do repositório (só tinha `.gitkeep`). **Agências = segmento de Rede** — pasta `04·Rede` › `04.1·Agências`; `09·Agências` está vazia/para arquivo. **Dentro de "04·Rede" há 4 pastas de segmento** (nested folders, 2026-07-01): `04.1·Agências` (`rede-seg-agencias`) · `04.2·Borda DC` (`rede-seg-bordadc`) · `04.3·DC Fabric` (`rede-seg-dcfabric`) · `04.4·Edifícios` (`rede-seg-edificios`); o N2 fica directo em `04·Rede`, fora de qualquer segmento. Cada `manifest.json` tem o `folderUid` correcto (aponta para a pasta de segmento, não para `04·Rede` directamente, excepto o N2). **Cuidado, achado 2026-07-10**: `push_panel.py`/`push_native.py` preservam sempre a pasta *actual* do dashboard no Grafana (lêem `resp['meta']['folderUid']`), nunca forçam o `folderUid` declarado no `manifest.json` — se um dashboard for criado sem pasta (ou movido manualmente), os pushes seguintes nunca o corrigem sozinhos; foi o que aconteceu com `apis-n3`/`apis-n4-sistema`, que ficaram na pasta `General` apesar do manifest dizer `07` — corrigido manualmente via API (`dashboards/db` com `folderUid` explícito). A pasta antiga "dashboards v5" (`efpbu5tvrhce8a`) está vazia/legada. Estrutura completa em `documentacao/engenharia-do-sistema.md §4.2` |
...
| **Nunca activar discovery rules/templates Zabbix, ou o mediatype `Email`, sem avaliar o risco de flood de triggers/notificações — mesmo que a escrita em si seja "só 1 mudança"** | Incidente real do utilizador (antes desta sessão, contexto trazido em 2026-07-04): activar um template MSSQL gerou uma **enxurrada de triggers → emails → tickets abertos automaticamente no GLPI**. **Mecanismo confirmado ao vivo em 2026-07-09** (`documentacao/auditoria-apis-servicos.md §4-bis`): **não há integração Zabbix→GLPI via API** — o GLPI abre 1 ticket por **cada email recebido** (email-to-ticket). O caminho é `trigger → action "Send Notification Email" (id 9, ON) → mediatype "Email" (id 1, hoje OFF) → caixa monitorizada pelo GLPI → 1 ticket`. Logo **ligar só o mediatype `Email`** (nenhuma mudança do lado do GLPI) já é suficiente para reactivar a criação automática de tickets em massa — é exactamente o que aconteceu no incidente. "Escrita pequena" (1 template/discovery rule/mediatype) não é sinónimo de "risco pequeno" — o impacto real está a jusante (quantas triggers/itens a mudança activa, se há `action` ligada, e se essa `action` usa um mediatype que dispara e-mail). **Antes de propor ou activar qualquer discovery rule, template, item desactivado, ou o mediatype `Email`**: (1) verificar explicitamente se há `action` ligada (`action.get`, leitura); (2) se houver e usar mediatype de email, confirmar com o utilizador o plano de reactivação (endereço/regras do GLPI, dependências/supressão de triggers para não abrir 1 ticket por trigger numa cascata, operação do dia-a-dia de triagem/fecho, teste com 1 trigger + 1 destinatário antes de alargar) — nunca assumir que a integração "continua morta" sem verificar; (3) preferir sempre testar em 1 host/trigger isolado antes de activar ao nível do template ou ligar o mediatype global. Nunca tratar "activar 1 discovery rule" ou "ligar 1 mediatype" como acção de baixo risco só porque tecnicamente é 1 chamada de API — o `service.discovery` nativo do template `Windows by Zabbix agent active` (achado e travado em `cronograma.md` Fase 14.16) fica **por decidir**, não activar sem este check. |
