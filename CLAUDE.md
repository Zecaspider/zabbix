...
> - **`documentacao/taxonomia-grupos-tags.md` — CONTRATO CANÓNICO de classificação de hosts (aprovado 2026-07-15)**: regra de ouro (grupo = domínio com pasta Grafana, `BPC/DOMINIO/NN Nome` em ASCII; tudo o resto = tags; subdomínio = tag `tipo`), racional domínio-vs-camadas, modelo-alvo de 8 grupos DOMINIO (inclui `08 Datacenter Fisico` novo para IBM Power/UCS/PowerFlex/PDUs e `10 Servicos de Suporte`), invariantes verificáveis (host activo ⇒ ≥1 grupo DOMINIO; VM ⇒ sempre em 03 + funcional), catálogo de tags canónicas, plano Fase A (colapso AMBIENTE/CAMADA/TECNOLOGIA/LOCALIZACAO, 0 refs em dashboards) / Fase B (renames+fusões+split do 603, refs medidas por grep: 609→32 ficheiros, 602→17) e decisões de classificação para casos futuros (OKD fica em 03, VMs de segurança em 03 por tag, Power em 08). **Ler antes de criar/classificar qualquer host**
