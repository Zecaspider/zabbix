...
(function initHeader() {
  ...
  // silenciosamente, cai no fallback estático). Substituído por leitura
  // directa do URL (mesmo mecanismo comprovado do objectContext do Borda
  // DC) + resolução assíncrona do nome do edifício (unidade_negocio) via
  // Zabbix, com cache por hostname para não repetir o RPC a cada tick.
  window._bpc_n5ed_labelCache = window._bpc_n5ed_labelCache || {};

  function n5edResolveHostLabel(host) {
    if (!host || window._bpc_n5ed_labelCache[host] !== undefined) return;
    window._bpc_n5ed_labelCache[host] = null;   // marca "a resolver"
    fetch(CFG_META.apiUrl, {
      ...
      .then(function (d) {
        ...
        window._bpc_n5ed_labelCache[host] = name
      })
      .catch(function () { window._bpc_n5ed_labelCache[host] = host })
  }
  ...
  function buildNocLabel() {
    ...
    n5edResolveHostLabel(host)
    const parts = [window._bpc_n5ed_labelCache[host] || host]
